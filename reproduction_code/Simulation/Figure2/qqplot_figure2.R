...
selectfeature<-function(Y1,X){
  cov<-(X[1:n,]-rowMeans(X[1:n,]))%*%t((X[1:n,]-rowMeans(X[1:n,])))/floor(n) #construct factors
  eigvec<-eigen(cov)$vectors
  eigval<-eigen(cov)$values
  K1<-which.min(diff(log(eigval[1:30]))) #select K
  hatf<-sqrt(n)*eigvec[,1:K1] #hatF
  hatB<-t(1/n*t(hatf)%*%X[1:n,]) #hat B
  hatU<-X[1:n,]-hatf%*%t(hatB) #hat U
  Y_new<-Y1[1:n]-1/floor(n)*hatf%*%t(hatf)%*%Y1[1:n] #
  sis<-SIS(hatU,Y_new)
  select1<-sis$ix #ISIS
  #S.hat0.ISIS = SIS0$ix; S.hat0.SIS = SIS0$ix0
  cov2<-(X[1:n,]-rowMeans(X[1:n,]))%*%t(X[1:n,]-rowMeans(X[1:n,]))/(n)
  eigvec2<-eigen(cov2)$vectors
  eigval2<-eigen(cov2)$values
  K<-which.min(diff(log(eigval2[1:30]))) #select K
  hatf2<-sqrt(n)*eigvec2[,1:K] #hat F using second half of data
  hatB2<-t(1/n*t(hatf2)%*%X[1:n,]) #hat B
  hatU2<-X[1:n,]-hatf2%*%t(hatB2) #hat U
  P_f<-hatf2%*%t(hatf2)/(n)
  P_x<-X[1:n,select1]%*%solve(t(X[1:n,select1])%*%(X[1:n,select1]))%*%t(X[1:n,select1])
  P_u<-hatU2[,select1]%*%solve(t(hatU2[,select1])%*%hatU2[,select1])%*%t(hatU2[,select1])
  Q1<-t(Y1[1:n])%*%(P_f+P_u-P_x)%*%(P_f+P_u-P_x)%*%Y1[1:n,]
  select2<-sis$ix0 #SIS
  P_x<-X[1:n,select2]%*%solve(t(X[1:n,select2])%*%(X[1:n,select2]))%*%t(X[1:n,select2])
  P_u<-hatU2[,select2]%*%solve(t(hatU2[,select2])%*%hatU2[,select2])%*%t(hatU2[,select2])
  Q2<-t(Y1[1:n])%*%(P_f+P_u-P_x)%*%(P_f+P_u-P_x)%*%Y1[1:n,]
...
for (m in c(0,0.6,1.2,1.8,2.4,3)){
  ...
  for(f in c(1:4)){
    ...
    for (i in 1:1000){
      ...
      for (r in c(1:n)){
        F_start=rnorm(K)#Phi%*%F_origin+rnorm(K)
        F_origin=F_start
        F[r,]<-F_start
      }
      ...
      Y2=F%*%gamma1+X%*%beta+rnorm(n,mean=0,sd=0.5)#runif(n,-sqrt(3)/2,sqrt(3)/2)#rnorm(n,mean=0,sd=0.5)#runif(n,-sqrt(3)/2,sqrt(3)/2)#rnorm(n,mean=0,sd=0.5) #alternative
    ...
    if ((f==1)&(m==0)){
      write.csv(isis1,"isis1.csv")
      write.csv(isis2,"isis2.csv")
      write.csv(isis3,"isis3.csv")
      write.csv(isis4,"isis4.csv")
      write.csv(isis5,"isis5.csv")
      write.csv(isis6,"isis6.csv")
    }
...
############## the QQ-Plot###############
no_split<-read.csv("~/Desktop/isis4.csv")
quantile_null<-no_split[,2]
length(quantile_null)

quantile_true<-rchisq(1000,3)

#hist(quantile_null)
#hist(quantile_true)


#hgA <- hist(quantile_null,plot = FALSE) 
#hgB<-hist(quantile_true,plot=FALSE)
#plot(hgA, col = 'lightblue')
#plot(hgB, col = 'lightpink', add = TRUE) 

qq<-qqplot(quantile_null, quantile_true,xlab="",ylab="")
