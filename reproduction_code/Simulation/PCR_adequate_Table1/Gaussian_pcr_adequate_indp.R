...
sum_idx13<-c()
  ...
  K<-which.min(diff(log(eigval[1:10]))) #select K
  hatf<-sqrt(n)*eigvec[,1:K]
  hatB<-t(1/n*t(hatf)%*%X)
  hatU<-X-hatf%*%t(hatB)
  ...
  c(sigma.hat.ISIS,sigma.hat.SIS)}
    ...
    Y=F%*%gamma1+rnorm(n,0,sigma)#runif(n,-sqrt(3)/2,sqrt(3)/2)#rnorm(n,0,sigma)#runif(n,-sqrt(3)/2,sqrt(3)/2)#rnorm(n,0,sigma)#runif(n,-sqrt(3)/2,sqrt(3)/2)#rnorm(n,0,sigma)#runif(n,-sqrt(3)/2,sqrt(3)/2) #null
