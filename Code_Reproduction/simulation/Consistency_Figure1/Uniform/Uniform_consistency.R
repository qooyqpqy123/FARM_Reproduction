...
for (r in ratio){
  ...
  for (j in 1:500){ #500 replications
    ...
    Y=F%*%gamma1+U%*%beta+runif(n,-sqrt(3)/2,sqrt(3)/2) #noise is generated following uniform distribution
  ...
  mat2[i,]<-dis2
  write.csv(mat,"mat.csv") #store the results for FARM
  write.csv(mat2,"mat2.csv") #store the results for LASSO
}
